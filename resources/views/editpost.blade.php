...
@section('content')
    ...
    <div class="container main-content">
        ...
        <div class="row">
            ...
            <!-----     Left Side     ------>
            <form method="POST" action="/posts/{{$posts->slug}}" enctype="multipart/form-data" class="md-form container-fluid row">
                @csrf
                @method ('PUT')
                    ...
                    <div class="container">
                        <div class="text-left" style="height:200px; background-color:grey; padding-top: 15%;  ">
                            @auth
                                ...
                                @endif
                            @endauth
                            <input class="text-center" value="{{$posts->image}}" type="file" name="image" id="image" style=" color: white; width: auto; position: absolute; margin-left: 40px;" >
                    ...
                    <div class="coment-input">
                        ...
                        <div class="form-group ">
                            ...
                            <div>
                                ...
                                <textarea class="form-control" name="title" id="title" placeholder="Post Title*..." >{{$posts->title}}</textarea>
                                @if($errors -> has('title'))
                                    <p style="color: red; background-color: white;">{{ $errors->first('title') }}</p>
                                @endif
                                <br>
                                <textarea class="form-control" id="body" name="body" rows="8" style="margin-top:15px;" placeholder="Post Description*..." >{{$posts->body}}</textarea>
                                @if($errors -> has('body'))
